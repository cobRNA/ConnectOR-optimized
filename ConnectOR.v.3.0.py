...
############################################################################################################
#######################################################################
statistic_1l = "Result/statistics/unidirectional/lncRNA/"
if os.path.exists(statistic_1l):
    shutil.rmtree(statistic_1l)
os.makedirs(statistic_1l)

statistic_1p = "Result/statistics/unidirectional/protein/"
if os.path.exists(statistic_1p):
    shutil.rmtree(statistic_1p)
os.makedirs(statistic_1p)

Orthologous_l1 = "Result/Orthologous/unidirectional/lncRNA/"
if os.path.exists(Orthologous_l1):
    shutil.rmtree(Orthologous_l1)
os.makedirs(Orthologous_l1)

Orthologous_p1 = "Result/Orthologous/unidirectional/protein/"
if os.path.exists(Orthologous_p1):
    shutil.rmtree(Orthologous_p1)
os.makedirs(Orthologous_p1)

unidirectional_plot = "plots/unidirectional/"
if os.path.exists(unidirectional_plot):
    shutil.rmtree(unidirectional_plot)
os.makedirs(unidirectional_plot)
...
#################                                                                                                   ###################
#######################################################################################################################################
            if gene_id not in one_to_one_sp1_A_B_map and sp1 == cleaned_filename:
                cleaned_filename2= sp1 + "to" + sp2
                if gene_biotype == 'lncRNA':
                    not_lifted_lncRNA.add(gene_id)
                if gene_biotype == 'pc':
                    not_lifted_pc.add(gene_id)

            if gene_id in one_to_one_sp1_A_B_map and sp1 == cleaned_filename:
                for gene_id_A, gene_biotype_A, gene_id_B, gene_biotype_B in one_to_one_sp1_A_B_map[gene_id]:
...
#################################################################################################################
                ...
                if gene_id_A_unic_no == 1 and gene_id_B_unic_no == 1:
                ...
                ################################################################################
                    ...
                    if gene_biotype == 'lncRNA':
                        #print(gene_id_A_unic_no, gene_id_B_unic_no, gene_id_A_unic, gene_id_B_unic)
                        gene_id_A_str = ' '.join([f"{id_} {biotype}" for id_, biotype in gene_id_A_unic])
                        ortholog_lncRNA.add(f"{gene_id_A_str} {tuple(gene_id_B_unic)}")
                    ...
                    if gene_biotype == 'pc':
                        gene_id_A_str = ' '.join([f"{id_} {biotype}" for id_, biotype in gene_id_A_unic])
                        ortholog_pc.add(f"{gene_id_A_str} {tuple(gene_id_B_unic)}")
...
#######################################################################################################################################
            ...
            if gene_id not in one_to_one_sp2_B_A_map and sp2 == cleaned_filename:
            ...
            if gene_id in one_to_one_sp2_B_A_map and sp2 == cleaned_filename:
                for gene_id_A, gene_biotype_A, gene_id_B, gene_biotype_B in one_to_one_sp2_B_A_map[gene_id]:
                    gene_id_A_unic.add((gene_id_A, gene_biotype_A))
...
#######################################################################################################################################
        ...
        not_lifted_lncRNA_no = len(not_lifted_lncRNA)
...
####################################################################################
        ...
        not_lifted_pc_no = len(not_lifted_pc)
...
###########################################################################################
        one_lncRNA_to_one_lncRNA_out = os.path.join(Orthologous_l1, "one_lncRNA_to_one_lncRNA_" + cleaned_filename2 + ".txt")
        lncRNA_one_to_one_out = os.path.join(Orthologous_l1, "one_to_one_" + cleaned_filename2 + ".txt")
        lncRNA_one_to_many_out = os.path.join(Orthologous_l1, "one_to_many_" + cleaned_filename2 + ".txt")

        with open(one_lncRNA_to_one_lncRNA_out, 'w') as output_file:
            for gene_info in lifted_one_one_lncRNA_lncRNA:
                output_file.write(f"{gene_info[0]}\t{gene_info[1]}\t{gene_info[2]}\t{gene_info[3]}\n")

        with open(lncRNA_one_to_one_out, 'w') as output_file:
            for gene_info in lifted_one_one_lncRNA:
                output_file.write(f"{gene_info[0]}\t{gene_info[1]}\t{gene_info[2]}\t{gene_info[3]}\n")

        with open(lncRNA_one_to_many_out, 'w') as output_file:
            for gene_info in ortholog_lncRNA:
                output_file.write(f"{gene_info}\n")
#############################################################

        pc_one_to_one_out = os.path.join(Orthologous_p1, "one_to_one_" + cleaned_filename2 + ".txt")
        one_protein_to_one_protein_out = os.path.join(Orthologous_p1, "one_lncRNA_to_one_protein_" + cleaned_filename2 + ".txt")
        pc_one_to_many_out = os.path.join(Orthologous_p1, "one_to_many_" + cleaned_filename2 + ".txt")

        with open(one_protein_to_one_protein_out, 'w') as output_file:
            for gene_info in lifted_one_one_pc_pc:
                output_file.write(f"{gene_info[0]}\t{gene_info[1]}\t{gene_info[2]}\t{gene_info[3]}\n")

        with open(pc_one_to_one_out, 'w') as output_file:
            for gene_info in lifted_one_one_pc:
                output_file.write(f"{gene_info[0]}\t{gene_info[1]}\t{gene_info[2]}\t{gene_info[3]}\n")

        with open(pc_one_to_many_out, 'w') as output_file:
            for gene_info in ortholog_pc:
                output_file.write(f"{gene_info}\n")

#####################################################################

        no_lncRNA2 = 0
        lifted_spesi_data_lncRNA = "lncRNA\tno_lncRNA\tlncRNA_liftover\tno_lncRNA_liftover\tOne_to_one_orth\tOne_to_many_orth\tOne_to_none_orth\tOne_lncRNA_to_one_lncRNA\tOne_lncRNA_to_one_protein\tOne_lncRNA_to_one_other\n"
        lifted_spesi_data_lncRNA = lifted_spesi_data_lncRNA + "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(total_lncRNA_no, no_lncRNA2, lifted_lncRNA_no, not_lifted_lncRNA_no, lifted_one_one_lncRNA_no, lifted_one_many_lncRNA_no,  lifted_one_none_lncRNA_no, lifted_one_one_lncRNA_lncRNA_no, lifted_one_one_lncRNA_pc_no, lifted_one_one_lncRNA_other_no)

        output_file_statistic_1l = os.path.join(statistic_1l, cleaned_filename2 + ".tsv")
        with open(output_file_statistic_1l, 'w') as output_file_stat1:
            output_file_stat1.write(f"{lifted_spesi_data_lncRNA}\n") 

        lifted_spesi_data_pc = "pc\tno_pc\tpc_liftover\tno_pc_liftover\tOne_to_one_orth\tOne_to_many_orth\tOne_to_none_orth\tOne_pc_to_one_pc\tOne_pc_to_one_lncRNA\tOne_pc_to_one_other\n"
        no_pc2 = 0
        lifted_spesi_data_pc = lifted_spesi_data_pc + "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(total_pc_no, no_pc2, lifted_pc_no, not_lifted_pc_no, lifted_one_one_pc_no, lifted_one_many_pc_no, lifted_one_none_pc_no, lifted_one_one_pc_pc_no, lifted_one_one_pc_lncRNA_no, lifted_one_one_pc_other_no)

        output_file_statistic_1p = os.path.join(statistic_1p, cleaned_filename2 + ".tsv")
        with open(output_file_statistic_1p, 'w') as output_file_stat2:
            output_file_stat2.write(f"{lifted_spesi_data_pc}\n") 

######################################################################

        #subprocess.call("Rscript ./scripts/pc_unidirectional.R", shell=True)
        #subprocess.call("Rscript ./scripts/lncRNA_unidirectional.R", shell=True)

        subprocess.call("Rscript ./scripts/pc_unidirectional.R "+str(gene_level)+" > /dev/null 2>/dev/null", shell=True)
        subprocess.call("Rscript ./scripts/lncRNA_unidirectional.R "+str(gene_level)+" > /dev/null 2>/dev/null", shell=True)
###########################################################################################################################
###########################################################################################################################
###############################                                                        ####################################
###############################               One Directional End                      ####################################                  
...
###########################################################################################
        ...
        with open(output_file_stat1, 'w') as output_file_stat1:
            output_file_stat1.write(f"{lifted_spesi_data_lncRNA}\n") 
        #subprocess.call("Rscript ./scripts/lncRNA_bi_directional.R", shell=True)
        subprocess.call("Rscript ./scripts/lncRNA_bi_directional.R "+str(gene_level)+" > /dev/null 2>/dev/null", shell=True)
        ...
        with open(output_file_stat2, 'w') as output_file_stat2:
            output_file_stat2.write(f"{lifted_spesi_data_pc}\n") 
        #subprocess.call("Rscript ./scripts/pc_bi_directional.R", shell=True)
        subprocess.call("Rscript ./scripts/pc_bi_directional.R "+str(gene_level)+" > /dev/null 2>/dev/null", shell=True)
