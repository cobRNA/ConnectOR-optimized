...
Orthologous_name_file = "Orthologous_map/"
if os.path.exists(Orthologous_name_file):
    shutil.rmtree(Orthologous_name_file)
os.makedirs(Orthologous_name_file)
...
for input_file2 in files:
    ...
    with open(file_path, 'r') as handle:
        ...
        lifted_lncRNA = set()
        lncRNA_to_lncRNA = set()
        lncRNA_orthology = set()
        pc_orthology = set()
        orthology = set()
        other_orthology = set()
...
#######################################################################################################################################
            ...
            if gene_id in all_sp1_sp2_map and sp1 == cleaned_filename:
                ...
                for gene_id_A, gene_biotype_A, gene_id_B, gene_biotype_B in all_sp1_sp2_map[gene_id]:
                    ...
                    if gene_id_A in all_sp1_sp2_map:
                        for gene_id3_A, gene_biotype3_A, gene_id3_B, gene_biotype3_B in all_sp1_sp2_map[gene_id_A]:
                            gene_id_A_unic.add((gene_id3_A, gene_biotype3_A))
                            if gene_id3_B != '.':
                                gene_id_B_unic.add((gene_id3_B, gene_biotype3_B))
...
#################################################################################################################
                ...
                if gene_id_A_unic_no == 1 and gene_id_B_unic_no == 1 and gene_id_A_half_no == 1 and gene_id_B_half_no == 1:
                    ...
                    for (gene_idA, gene_biotypeA), (gene_idB, gene_biotypeB) in zip(gene_id_A_unic, gene_id_B_unic):
                        ...
                        if gene_id == gene_idA and gene_biotypeA == 'lncRNA':
                            lifted_one_one_lncRNA.add((gene_idA, gene_biotypeA, gene_idB, gene_biotypeB))
                            orthology.add((gene_idA, gene_biotypeA, gene_idB, gene_biotypeB, 'one-to-one'))
                            if gene_biotypeB == 'lncRNA':
                                lifted_one_one_lncRNA_lncRNA.add((gene_idA, gene_biotypeA, gene_idB, gene_biotypeB))
                                lncRNA_to_lncRNA.add((gene_idA, gene_biotypeA, gene_idB, gene_biotypeB, 'one-to-one'))
                        ...
                        if gene_id == gene_idA and gene_biotypeA == 'pc':
                ...
                ################################################################################
                    ...
                    if gene_id_A_half_no == 1 and gene_id_B_half_no == 0:
                        ...
                        for (gene_idA, gene_biotypeA), (gene_idB, gene_biotypeB) in zip(gene_id_A_unic, gene_id_B_unic):
                            ...
                            if gene_id == gene_idA and gene_biotypeA == 'lncRNA':
                                orthology.add((gene_idA, gene_biotypeA, gene_idB, gene_biotypeB, 'one-to-half'))
                ...
                ################################################################################
                    ...
                    for (gene_idA, gene_biotypeA) in (gene_id_A_unic):
                        if gene_id == gene_idA:
                            for (gene_idB, gene_biotypeB) in (gene_id_B_unic):
                                if gene_biotypeB in ['lncRNA', 'pc', 'ncRNA', 'other']:
                                    orthology.add((gene_idA, gene_biotypeA, gene_idB, gene_biotypeB, 'one-to-many'))
                    ...
                    for (gene_idA, gene_biotypeA) in (gene_id_A_unic):
                        if gene_id == gene_idA:
                            for (gene_idB, gene_biotypeB) in (gene_id_B_unic):
                                if gene_biotypeB in ['lncRNA', 'pc', 'ncRNA', 'other']:
                                    orthology.add((gene_idA, gene_biotypeA, gene_idB, gene_biotypeB, 'many-to-many'))
                    if gene_biotype == 'lncRNA':
...
#############################################################
        ...
        orthology_out = os.path.join(Orthologous_name_file, "orthology.table." + cleaned_filename2 + ".tsv")
 
                
        with open(orthology_out, 'w') as output_file2:
            output_file2.write(f"{sp11}\t{sp11} gene type\t{sp22}\t{sp22} gene type\torthology type\n") 
            for gene_info in orthology:
                output_file2.write(f"{gene_info[0]}\t{gene_info[1]}\t{gene_info[2]}\t{gene_info[3]}\t{gene_info[4]}\n") 
###########################################################################################
