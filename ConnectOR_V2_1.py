...
### Dictionaris
with open('dictionaries_v2_1.json') as f:
...
"non_coding", "sense_intronic", "sense_overlapping", "macro_lncRNA", 
...
by_spesi_data = "Species\tprotein\tlncRNA\tncRNA\tother\n"
folder_path = "BEDs/"
...
for filename in file_list:
    ...
    with open(file_path, 'r') as handle:
        for line in handle:
            ...
            if gene_name in ["lincRNA", "lncRNA"]:
                unique_gene_ids.add('\t'.join([gene_id, 'lncRNA']))
                unique_gene_lncRNA.add(gene_id)

            elif gene_type in ["lincRNA", "lncRNA"]:
                unique_gene_ids.add('\t'.join([gene_id, 'lncRNA']))
                unique_gene_lncRNA.add(gene_id)

            elif gene_type in ["ncRNA"]:
                unique_gene_ids.add('\t'.join([gene_id, 'ncRNA']))
                unique_gene_ncRNA.add(gene_id)

            elif gene_type in ["protein_coding", "pc"]:
                unique_gene_ids.add('\t'.join([gene_id, 'protein_coding']))
                unique_gene_protein.add(gene_id)
            else:
                unique_gene_ids.add('\t'.join([gene_id, 'other']))
                unique_gene_other.add(gene_id)
        #output_file.write('\n'.join(unique_gene_ids) + '\n')
        ...
        by_spesi_data = by_spesi_data + "{}\t{}\t{}\t{}\t{}\n".format(cleaned_filename, protein_id_no, lncRNA_id_no, ncRNA_id_no, outher_id_no)
        #by_spesi_data = by_spesi_data + "{}\t{}\t{}\t{}\t{}\t{}\n".format(cleaned_filename, all_id_no, protein_id_no, lncRNA_id_no, ncRNA_id_no, outher_id_no)
...
output_file_stat1 = os.path.join("Result/ortholog.tsv")

output_file_stat2 = os.path.join("Result/stat.tsv")
#by_spesi_data = "Species\texon gene\tprotein\tlncRNA\tncRNA\tother \n"
lifted_spesi_data2 = "Species1\tSp1_to_sp2\tprotein_lifted\tlncRNA_lifted\tncRNA_lifted\n"

lifted_spesi_data = "Sp1_to_sp2\tprotein_ortholog\tprotein_lncRNA_ncRNA_ortholog\tprotein_intergenic\tlncRNA_ortholog\tlncRNA_protein_ncRNA_ortholog\tlncRNA_intergenic\tncRNA_ortholog\tncRNA_protein_lncRNA_ortholog\tncRNA_intergenic\n"
...
for input_file in files:
    cleaned_filename = input_file.replace('.exons', '')
    ...
    unique_pc_ids = set()
    unique_pc_pc = set()
    unique_pc_lncRNA = set()
    unique_pc_ncRNA = set()
    unique_pc_other = set()
    unique_pc_none = set()
    unique_pc_mult = set()

    unique_lncRNA_ids = set()
    unique_lncRNA_lncRNA = set()
    unique_lncRNA_pc = set()
    unique_lncRNA_ncRNA = set()
    unique_lncRNA_other = set()
    unique_lncRNA_none = set()
    unique_lncRNA_mult = set()

    unique_ncRNA_ids = set()
    unique_ncRNA_ncRNA = set()
    unique_ncRNA_lncRNA = set()
    unique_ncRNA_pc = set()
    unique_ncRNA_other = set()
    unique_ncRNA_none = set()
    unique_ncRNA_mult = set()
    ...
    with open(f'{input_path}/{input_file}', 'r') as handle:
        ...
        for line in handle:
            ...
            gene_id = line[0]
            gene_type = line[1]
            unique_lift_ids.add(gene_id)

            geneB_type_index_count = 0
            geneB_type_index_pc_count = 0
            geneB_type_index_ncRNA_count = 0
            geneB_type_index_lncRNA_count = 0
            geneB_type_index_other_count = 0
            geneB_type_index_count_none = 0
            geneB_type_elements = line[3].split(",")
            
            #print(geneB_type_elements, "\n")
            for geneB_type_index in geneB_type_elements:
                #unique_ids.add(geneB_type_index)
                if geneB_type_index == 'pc':
                    geneB_type_index_pc_count = 1
                    #geneB_type_index_count += 1
                if geneB_type_index == 'ncRNA':
                    geneB_type_index_ncRNA_count = 1
                    #geneB_type_index_count += 1
                if geneB_type_index == 'lncRNA':
                    geneB_type_index_lncRNA_count = 1
                    #geneB_type_index_count += 1
                if geneB_type_index == 'other':
                    geneB_type_index_other_count = 1
                    #geneB_type_index_count += 1
                if geneB_type_index == 'none':
                    geneB_type_index_count_none = 1
                    #geneB_type_index_count += 1
            geneB_type_index_count = (
                geneB_type_index_pc_count +
                geneB_type_index_ncRNA_count +
                geneB_type_index_lncRNA_count +
                geneB_type_index_other_count +
                geneB_type_index_count_none
            )
            #print("\n",geneB_type_index_count,gene_type, geneB_type_elements)
#######################################################################################################
            if gene_type == 'pc':
                unique_pc_ids.add(gene_id)
            if gene_type == 'pc' and geneB_type_index_pc_count == 1 and geneB_type_index_count == 1:
                unique_pc_pc.add(gene_id)

            if gene_type == 'pc' and geneB_type_index_lncRNA_count == 1 and geneB_type_index_count == 1:
                unique_pc_lncRNA.add(gene_id)

            if gene_type == 'pc' and geneB_type_index_ncRNA_count == 1 and geneB_type_index_count == 1:
                unique_pc_ncRNA.add(gene_id)

            if gene_type == 'pc' and geneB_type_index_other_count == 1 and geneB_type_index_count == 1:
                unique_pc_other.add(gene_id)

            if gene_type == 'pc' and geneB_type_index_count > 1:
                unique_pc_mult.add(gene_id)

            if gene_type == 'pc' and geneB_type_index_count_none == 1 and geneB_type_index_count == 1:
                unique_pc_none.add(gene_id)

#######################################################################################################
            if gene_type == 'lncRNA':
                unique_lncRNA_ids.add(gene_id)
            if gene_type == 'lncRNA' and geneB_type_index_pc_count == 1 and geneB_type_index_count == 1:
                unique_lncRNA_pc.add(gene_id)

            if gene_type == 'lncRNA' and geneB_type_index_lncRNA_count == 1 and geneB_type_index_count == 1:
                unique_lncRNA_lncRNA.add(gene_id)

            if gene_type == 'lncRNA' and geneB_type_index_ncRNA_count == 1 and geneB_type_index_count == 1:
                unique_lncRNA_ncRNA.add(gene_id)

            if gene_type == 'lncRNA' and geneB_type_index_other_count == 1 and geneB_type_index_count == 1:
                unique_lncRNA_other.add(gene_id)

            if gene_type == 'lncRNA' and geneB_type_index_count > 1:
                unique_lncRNA_mult.add(gene_id)

            if gene_type == 'lncRNA' and geneB_type_index_count_none == 1 and geneB_type_index_count == 1:
                unique_lncRNA_none.add(gene_id)

#######################################################################################################
            if gene_type == 'ncRNA':
                unique_ncRNA_ids.add(gene_id)
            if gene_type == 'ncRNA' and geneB_type_index_pc_count == 1 and geneB_type_index_count == 1:
                unique_ncRNA_pc.add(gene_id)

            if gene_type == 'ncRNA' and geneB_type_index_lncRNA_count == 1 and geneB_type_index_count == 1:
                unique_ncRNA_lncRNA.add(gene_id)

            if gene_type == 'ncRNA' and geneB_type_index_ncRNA_count == 1 and geneB_type_index_count == 1:
                unique_ncRNA_ncRNA.add(gene_id)

            if gene_type == 'ncRNA' and geneB_type_index_other_count == 1 and geneB_type_index_count == 1:
                unique_ncRNA_other.add(gene_id)

            if gene_type == 'ncRNA' and geneB_type_index_count > 1:
                unique_ncRNA_mult.add(gene_id)

            if gene_type == 'ncRNA' and geneB_type_index_count_none == 1 and geneB_type_index_count == 1:
                unique_ncRNA_none.add(gene_id)



#######################################################################################################

        unique_lift_ids_no = len(unique_lift_ids)

        unique_pc_ids_no = len(unique_pc_ids)
        unique_pc_pc_no = len(unique_pc_pc)
        unique_pc_lncRNA_no = len(unique_pc_lncRNA)
        unique_pc_ncRNA_no = len(unique_pc_ncRNA)
        unique_pc_other_no = len(unique_pc_other)
        unique_pc_none_no = len(unique_pc_none)
        unique_pc_mult_no = len(unique_pc_mult)
        pc_marge= unique_pc_lncRNA_no + unique_pc_ncRNA_no + unique_pc_other_no + unique_pc_mult_no

        unique_lncRNA_ids_no = len(unique_lncRNA_ids)
        unique_lncRNA_lncRNA_no = len(unique_lncRNA_lncRNA)
        unique_lncRNA_ncRNA_no = len(unique_lncRNA_ncRNA)
        unique_lncRNA_pc_no = len(unique_lncRNA_pc)
        unique_lncRNA_other_no = len(unique_lncRNA_other)
        unique_lncRNA_none_no = len(unique_lncRNA_none)
        unique_lncRNA_mult_no = len(unique_lncRNA_mult)
        lncRNA_marge= unique_lncRNA_ncRNA_no + unique_lncRNA_pc_no + unique_lncRNA_other_no + unique_lncRNA_mult_no

        unique_ncRNA_ids_no = len(unique_ncRNA_ids)
        unique_ncRNA_ncRNA_no = len(unique_ncRNA_ncRNA)
        unique_ncRNA_lncRNA_no = len(unique_ncRNA_lncRNA)
        unique_ncRNA_pc_no = len(unique_ncRNA_pc)
        unique_ncRNA_other_no = len(unique_ncRNA_other)
        unique_ncRNA_none_no = len(unique_ncRNA_none)
        unique_ncRNA_mult_no = len(unique_ncRNA_mult)
        ncRNA_marge = unique_ncRNA_lncRNA_no + unique_ncRNA_pc_no + unique_ncRNA_other_no + unique_ncRNA_mult_no

#######################################################################################################

        lifted_spesi_data2 = lifted_spesi_data2 + "{}\t{}\t{}\t{}\t{}\n".format(Species1,cleaned_filename, unique_pc_ids_no, unique_lncRNA_ids_no, unique_ncRNA_ids_no)

        lifted_spesi_data = lifted_spesi_data + "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(cleaned_filename, unique_pc_pc_no, pc_marge, unique_pc_none_no, unique_lncRNA_lncRNA_no, lncRNA_marge, unique_lncRNA_none_no, unique_ncRNA_ncRNA_no, ncRNA_marge, unique_ncRNA_none_no)
...
file_path2 = 'Result/stat.tsv'
######################################################################
output_file_stat1 = os.path.join("Result/liftover.tsv")

#by_spesi_data = "Species\texon gene\tprotein\tlncRNA\tncRNA\tother \n"
#lifted_spesi_data = "Sp1\tSp1_to_sp2\ttotal_gene\tprotein\tlncRNA\tncRNA\tother\tprotein_lifted\tnot_lifted_protein\tlncRNA_lifted\tnot_lifted_lncRNA\tncRNA_lifted\tnot_lifted_ncRNA\tpc_pc\tpc_lncRNA\tpc_ncRNA\tpc_other\tpc_none\tpc_mult\tlncRNA_lncRNA\tlncRNA_ncRNA\tlncRNA_pc\tlncRNA_other\tlncRNA_none\tlncRNA_mult\tncRNA_ncRNA\tncRNA_lncRNA\tncRNA_pc\tncRNA_other\tncRNA_none\tncRNA_mult\n"
